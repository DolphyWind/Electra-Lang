...
bool FileCloser::work(Current::Ptr current, std::vector<Current::Ptr>& currentVector)
{
    ...
    {
        defaultLogger.log(LogType::WARNING, "Stack is empty! There are no file id to close.");
    ...
    {
        defaultLogger.log(LogType::WARNING, "Unable to close file with id {}. Pushing 0 to stack.", id);
        current->stackPtr->push(0);
    }
    else
    {
        defaultLogger.log(LogType::INFO, "Closed file with id {}.", id);
    }
